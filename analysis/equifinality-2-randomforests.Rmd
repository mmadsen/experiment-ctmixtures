...
two_locus_model <- do_random_forest(eq2_pop_df, "two_class_label", excluded_fields)
...
# exclude the original four class labels from the random forest classifier
excluded_fields <- c("simulation_run_id", "model_class_label", "innovation_rate")
low_two_model <- do_random_forest(low_innov_df, "two_class_label", excluded_fields)
...
```{r low-num-configs, results='asis', echo=FALSE}
ggplot(low_innov_df, aes(x = num_trait_configurations, y = ..density..)) + geom_density(aes(linetype=model_class_label, position="stack"))
```
