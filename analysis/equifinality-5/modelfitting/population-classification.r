## FILE: population-classification.r
...
log_file <- "/mnt/experiment-ctmixtures/equifinality-5/population-classification.log"
...
# test_indices <- createDataPartition(eq5_pop_df$model_class_label, p = 0.005, list=FALSE)
# eq5_pop_df <- eq5_pop_df[test_indices,]
# flog.info("Test downsample to %.0f rows", nrow(eq5_pop_df), name='cl')
...
exclude_columns <- c("simulation_run_id", "innovation_rate", "model_class_label")


# fit GBM model
model <- train_gbm_classifier(eq5_neutral_conformist_df, training_set_fraction, "two_class_label", gbm_grid, training_control, exclude_columns, verbose=FALSE)
...
# fit GBM model
model <- train_gbm_classifier(eq5_neutral_anticonformist_df, training_set_fraction, "two_class_label", gbm_grid, training_control, exclude_columns, verbose=FALSE)
