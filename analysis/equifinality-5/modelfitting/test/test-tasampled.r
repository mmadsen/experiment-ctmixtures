...
if(length(clargs) == 0) {
  ta_sampled_data_file <- get_data_path(suffix = "experiment-ctmixtures/equifinality-5", filename = "equifinality-5-tasampled-data.rda")
} else {
  ta_sampled_data_file <- get_data_path(suffix = "experiment-ctmixtures/equifinality-5", filename = "equifinality-5-tasampled-data.rda", args = clargs)
}
...
flog.info("Beginning classification analysis of TA sampled equifinality-5 data sets", name='cl')
...
eq5_ta_sampled_df$two_class_label <- factor(ifelse(eq5_ta_sampled_df$model_class_label == 'allneutral', 'neutral', 'biased'))
...
test_tasampled_indices <- createDataPartition(eq5_ta_sampled_df$two_class_label, p = 0.02, list=FALSE)
test_tasampled_df <- eq5_ta_sampled_df[test_tasampled_indices,]
# switch the DF input to get_tassize_subset_ssize_tadur() back to eq5_ta_sampled_df for production
