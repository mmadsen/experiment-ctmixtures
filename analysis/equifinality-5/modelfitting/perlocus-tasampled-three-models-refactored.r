## FILE: perlocus-tasampled-three-models-refactored.r
...
perlocus_neutral_conformist_results <- data.frame()
perlocus_neutral_conformist_roc <- NULL
perlocus_neutral_conformist_roc_ssize_20 <- NULL
perlocus_neutral_conformist_roc_ssize_10 <- NULL
perlocus_neutral_conformist_model <- NULL
perlocus_neutral_conformist_cm <- NULL

experiment_names <- get_experiment_names("Per-Locus Neutral vs Conformist", tassize_subsets)
comparison <- "Neutral versus Conformist"

for( i in 1:nrow(tassize_subsets)) {
  exclude_columns <- c("simulation_run_id", "innovation_rate", "model_class_label", "sample_size", "ta_duration","configuration_slatkin", "num_trait_configurations")
  ...
  perlocus_neutral_conformist_cm[[exp_name]] <- result$cm
  perlocus_neutral_conformist_model[[exp_name]] <- result$model
  perlocus_neutral_conformist_roc[[exp_name]] <- result$roc
  perlocus_neutral_conformist_results <- rbind(perlocus_neutral_conformist_results, result$stats)
  if(ssize == 20) {
    perlocus_neutral_conformist_roc_ssize_20[[exp_name]] <- result$roc
  }
  if(ssize == 10) {
    perlocus_neutral_conformist_roc_ssize_10[[exp_name]] <- result$roc
  }
...
perlocus_neutral_anticonformist_results <- data.frame()
perlocus_neutral_anticonformist_roc <- NULL
perlocus_neutral_anticonformist_roc_ssize_20 <- NULL
perlocus_neutral_anticonformist_roc_ssize_10 <- NULL
perlocus_neutral_anticonformist_model <- NULL
perlocus_neutral_anticonformist_cm <- NULL

experiment_names <- get_experiment_names("Per-Locus Neutral vs Anticonformist", tassize_subsets)
...
for( i in 1:nrow(tassize_subsets)) {
  ...
  subset_df <- get_tassize_subset_ssize_tadur(eq5_neutral_anticonformist_df,
                                              ...
                                              ta_dur)
  result <- do_model_fit_and_test(comparison,experiment_names[i], ssize, ta_dur, target_label, subset_df, gbm_grid, training_control, exclude_columns)
  perlocus_neutral_anticonformist_cm[[exp_name]] <- result$cm
  perlocus_neutral_anticonformist_model[[exp_name]] <- result$model
  perlocus_neutral_anticonformist_roc[[exp_name]] <- result$roc
  perlocus_neutral_anticonformist_results <- rbind(perlocus_neutral_anticonformist_results, result$stats)
  if(ssize == 20) {
    perlocus_neutral_anticonformist_roc_ssize_20[[exp_name]] <- result$roc
  }
  if(ssize == 10) {
    perlocus_neutral_anticonformist_roc_ssize_10[[exp_name]] <- result$roc
  }
  logrow <- sprintf("row %.0f:  sample size: %2.0f  ta duration: %3.0f numrows: %.0f  elapsed: %.4f", i, ssize, ta_dur, nrow(subset_df), result$elapsed)
  flog.info("%s", logrow, name='cl')
}
...
perlocus_neutral_biased_results <- data.frame()
perlocus_neutral_biased_roc <- NULL
perlocus_neutral_biased_roc_ssize_20 <- NULL
perlocus_neutral_biased_roc_ssize_10 <- NULL
perlocus_neutral_biased_model <- NULL
perlocus_neutral_biased_cm <- NULL

experiment_names <- get_experiment_names("Per-Locus Neutral vs Biased", tassize_subsets)
...
for( i in 1:nrow(tassize_subsets)) {
  ...
  subset_df <- get_tassize_subset_ssize_tadur(eq5_ta_sampled_df,
                                              ...
                                              ta_dur)
  result <- do_model_fit_and_test(comparison,experiment_names[i], ssize, ta_dur, target_label, subset_df, gbm_grid, training_control, exclude_columns)
  perlocus_neutral_biased_cm[[exp_name]] <- result$cm
  perlocus_neutral_biased_model[[exp_name]] <- result$model
  perlocus_neutral_biased_roc[[exp_name]] <- result$roc
  perlocus_neutral_biased_results <- rbind(perlocus_neutral_biased_results, result$stats)
  if(ssize == 20) {
    perlocus_neutral_biased_roc_ssize_20[[exp_name]] <- result$roc
  }
  if(ssize == 10) {
    perlocus_neutral_biased_roc_ssize_10[[exp_name]] <- result$roc
  }
  logrow <- sprintf("row %.0f:  sample size: %2.0f  ta duration: %3.0f numrows: %.0f  elapsed: %.4f", i, ssize, ta_dur, nrow(subset_df), result$elapsed)
  flog.info("%s", logrow, name='cl')
}
...
if(length(clargs) == 0) {
  image_file <- get_data_path(suffix = "experiment-ctmixtures/equifinality-5",
                              filename = "perlocus-ta-sampled-results-gbm.RData")
  image_file_results <- get_data_path(suffix = "experiment-ctmixtures/equifinality-5",
                                      filename = "perlocus-ta-sampled-results-gbm-dfonly.RData")
} else {
  image_file <- get_data_path(suffix = "experiment-ctmixtures/equifinality-5",
                              filename = "perlocus-ta-sampled-results-gbm.RData", args = clargs)
  image_file_results <- get_data_path(suffix = "experiment-ctmixtures/equifinality-5",
                                      filename = "perlocus-ta-sampled-results-gbm-dfonly.RData", args = clargs)
}
flog.info("Saving results of analysis to R environment snapshot: %s", image_file, name='cl')
save(perlocus_neutral_conformist_cm,
    perlocus_neutral_conformist_results,
    perlocus_neutral_conformist_model,
    perlocus_neutral_conformist_roc,
    perlocus_neutral_conformist_roc_ssize_10,
    perlocus_neutral_conformist_roc_ssize_20, 
    perlocus_neutral_anticonformist_results,
    perlocus_neutral_anticonformist_model,
    perlocus_neutral_anticonformist_roc,
    perlocus_neutral_anticonformist_roc_ssize_20,
    perlocus_neutral_anticonformist_roc_ssize_10,
    perlocus_neutral_anticonformist_cm,
    perlocus_neutral_biased_results,
    perlocus_neutral_biased_model,
    perlocus_neutral_biased_roc,
    perlocus_neutral_biased_roc_ssize_20,
    perlocus_neutral_biased_roc_ssize_10,
...
save(perlocus_neutral_conformist_results,perlocus_neutral_anticonformist_results,perlocus_neutral_biased_results, file=image_file_results)
# End
