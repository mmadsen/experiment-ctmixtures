...
\begin{figure}[ht]
\centering
\includegraphics{figure/distributional-overlap-scaled.pdf}
...
Answering both questions can be done through simulation of outcomes from each cultural transmission model one plans to use in an empirical study.  The general process is:  
...
The remainder of the paper provides a detailed example of this process, and in the process evaluates the equifinality of several transmission models commonly employed in archaeological research.  
...
This set of models was chosen because much of the archaeological literature on modes of cultural transmission has centered around the ability to identify biased transmission, using neutral models as null hypotheses.  Mixtures of biased models were chosen because (a) they present a strong chance of demonstrating equifinality, and (b) they represent more realistic models of a natural human population than do "pure strategy" models of the type usually studied in the literature.  A population with no bias to common or uncommon traits might, when viewed in the aggregate, look much like a population composed of 50% individuals who prefer the most common trait, and 50% who prefer the least common, for example.  

A question thus arises:  given the type of data available to archaeologists (in most cases), is it possible to distinguish a mixture of transmission biases from unbiased copying?  Is it possible to distinguish a situation dominated by conformists, from a situation dominated by anti-conformists?  If we can, given sample sizes and time averaging, then fitting such models to archaeological data will continue to be a fruitful research strategy.  If we find significant equifinality, then perhaps we should focus attention on other types of questions connected with transmission in past societies.  A subsidiary question relates to the variables we measure:  which measures of richness or diversity are effective at separating our transmission models?  
...
```{r load-unbiased-allbias, echo=FALSE, cache=FALSE, message=FALSE}

# Load file containing data frame "classifier_results," which has 16 summary statistics for 19 data collection treatments, 
# comparing unbiased to all biased models

load(get_data_path(suffix="experiment-ctmixtures/equifinality-4/results", filename = "classification-gbm-merged-dfonly.RData"))

# loads object cm_objects, whose keys are experiment names, and which contains the Caret confusion matrix object 
# for unbiased compared to all biased models.  
