...
While regression models are evaluated by calculating the mean squared error of their predicts, classification models are usually assessed using the "0-1" loss function, which counts the number of misclassified data points when the true generating class is known.  The basic data for calculating accuracy is the _confusion matrix_, which compares classification successes and errors for a data set (an example is given in Table \ref{tab:confusion-matrix}).  
